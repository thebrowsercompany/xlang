...
inline void write_swift_fully_qualified_type(writer& w, std::string_view typeNamespace, std::string_view typeName)
{
...
{
    return xlang::text::bind<write_swift_fully_qualified_type>(typeNamespace, typeName);
}
...
inline std::array<char, 256> type_iid_expanded(xlang::meta::reader::TypeDef const& type)
{
    ...
    // Data1: 0x320b7e22, Data2: 0x3cb0, Data3: 0x4cdf, Data4: (0x86, 0x63, 0x1d, 0x28, 0x91, 0x00, 0x65, 0xeb)
    std::snprintf(result.data(), result.size(), "Data1: 0x%08x, Data2: 0x%04x, Data3: 0x%04x, Data4: (0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x)",
...
inline void write_uuid_expanded(writer& w, xlang::meta::reader::TypeDef const& type)
{
    auto iidStr = type_iid_expanded(type);
    w.write(iidStr.data());
}

inline void write_uuid_expanded(writer& w, typedef_base const& type)
{
    write_uuid_expanded(w, type.type());
}

template <typename T>
auto bind_uuid_expanded(T const& type)
{
    return [&](writer& w)
    {
        write_uuid_expanded(w, type);
    };
}
